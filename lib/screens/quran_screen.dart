    ...
    setState(() {
      ...
      for (var juz in QuranData.parts) {
        for (var surah in juz.surahs) {
          ...
          // Determine the correct starting verse number based on the Juz
          ...
          for (int i = 0; i < surah.versesAr.length; i++) {
            ...
            if (cleanVerseAr.contains(cleanQuery) || verseEn.contains(cleanQuery)) {
              _searchResults.add({
                'surah': surah,
                'verseIndex': i,
                'trueVerseNumber': i + startNumber, // Correct global number
                'startVerseNumber': startNumber, // Stored to pass to Reader Screen
        ...
        return _SearchResultGlassCard(
          ...
          onTap: () {
            Navigator.push(context, MaterialPageRoute(builder: (context) => SurahReaderScreen(
              ...
              startVerseNumber: startNumber, // Now passes correctly based on search map
            )));
          },
        );
      },
...
class _SearchResultGlassCard extends StatefulWidget {
  ...
  const _SearchResultGlassCard({required this.searchResult, required this.lang, required this.onTap});
            ...
            child: ClipRRect(
              ...
              child: BackdropFilter(
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(color: _isHovered ? const Color(0xFF144D32).withValues(alpha: 0.8) : const Color(0xFF1B5E3F).withValues(alpha: 0.5), borderRadius: BorderRadius.circular(16), border: Border.all(color: _isHovered ? const Color(0xFFD4AF37) : const Color(0xFFD4AF37).withValues(alpha: 0.4), width: _isHovered ? 2 : 1)),
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(verseAr, maxLines: 2, overflow: TextOverflow.ellipsis, textDirection: TextDirection.rtl, style: GoogleFonts.amiri(color: Colors.white, fontSize: 22, height: 1.8)),
                    ],
                  ),
                ),
              ),
        ...
        child: AnimatedScale(
          ...
          child: ClipRRect(
            ...
            child: BackdropFilter(
              ...
              child: AnimatedContainer(
                duration: const Duration(milliseconds: 200),
                decoration: BoxDecoration(color: _isHovered ? const Color(0xFF144D32).withValues(alpha: 0.8) : const Color(0xFF0B3D2E).withValues(alpha: 0.65), borderRadius: BorderRadius.circular(20), border: Border.all(color: _isHovered ? const Color(0xFFD4AF37).withValues(alpha: 0.8) : const Color(0xFFD4AF37).withValues(alpha: 0.3), width: _isHovered ? 2 : 1)),
                child: Center(
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Text('الجزء', style: GoogleFonts.amiri(color: _isHovered ? Colors.white : const Color(0xFFD4AF37), fontSize: 20)),
                      Text('${widget.part.id}', style: TextStyle(color: _isHovered ? const Color(0xFFD4AF37) : Colors.white, fontSize: 42, fontWeight: FontWeight.bold)),
                      const SizedBox(height: 4),
                      Flexible(child: Text(widget.lang == 'ar' ? widget.part.titleAr : widget.part.titleEn, textAlign: TextAlign.center, overflow: TextOverflow.ellipsis, style: GoogleFonts.elMessiri(color: Colors.white.withValues(alpha: 0.8), fontSize: 14))),
                    ],
                  ),
                ),
              ),
                ...
                child: Center(
                  child: ConstrainedBox(
                    ...
                    child: GridView.builder(
                      ...
                      itemBuilder: (context, index) {
                        return _SurahGlassCard(surah: widget.part.surahs[index], lang: widget.lang, part: widget.part);
                      },
                    ),
                  ),
                ),
...
class _SurahGlassCard extends StatefulWidget {
  ...
  final String lang;
  final QuranJuz part;
  const _SurahGlassCard({required this.surah, required this.lang, required this.part});
  @override
          ...
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) => SurahReaderScreen(
                surah: widget.surah, 
                lang: widget.lang, 
                startVerseNumber: (widget.part.id == 2) ? 142 : 1, // Start logic maintained
              ),
            ),
          );
        },
          ...
          child: ClipRRect(
            ...
            child: BackdropFilter(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(color: _isHovered ? const Color(0xFF144D32).withValues(alpha: 0.8) : const Color(0xFF0B3D2E).withValues(alpha: 0.6), borderRadius: BorderRadius.circular(20), border: Border.all(color: _isHovered ? const Color(0xFFD4AF37).withValues(alpha: 0.8) : const Color(0xFFD4AF37).withValues(alpha: 0.3), width: _isHovered ? 2 : 1)),
                child: Column(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    Container(
                    ...
                    FittedBox(
                      ...
                      child: Text(widget.surah.nameAr, textAlign: TextAlign.center, style: GoogleFonts.amiri(color: _isHovered ? Colors.white : const Color(0xFFD4AF37), fontSize: 24, fontWeight: FontWeight.bold)),
                    ),
                    const SizedBox(height: 4),
                    Flexible(
                      child: Text(widget.surah.nameEn, textAlign: TextAlign.center, overflow: TextOverflow.ellipsis, maxLines: 2, style: GoogleFonts.elMessiri(color: Colors.white.withValues(alpha: 0.7), fontSize: 13)),
                    ),
                  ],
                ),
              ),
...
class _SurahReaderScreenState extends State<SurahReaderScreen> {
  ...
  Widget build(BuildContext context) {
    String appBarTitle;
    if (widget.lang == 'ar') appBarTitle = 'سورة ${widget.surah.nameAr}';
                          ...
                          children: [
                            ...
                            ...List.generate(widget.surah.versesAr.length, (index) {
                              ...
                              if (index < widget.surah.versesEn.length) {
                                if (widget.lang == 'fr') translatedVerse = _getFrenchVerse(widget.surah.id, index, widget.surah.versesEn[index]);
                                else translatedVerse = widget.surah.versesEn[index];
                              }

                              return Container(
                                key: verseKey,
                                margin: const EdgeInsets.only(bottom: 16),
                                decoration: BoxDecoration(color: isHighlighted ? const Color(0xFF1B5E3F).withValues(alpha: 0.7) : const Color(0xFF0B3D2E).withValues(alpha: 0.4), borderRadius: BorderRadius.circular(16), border: Border.all(color: isHighlighted ? const Color(0xFFD4AF37) : const Color(0xFFD4AF37).withValues(alpha: 0.15), width: isHighlighted ? 2 : 1)),
                                padding: const EdgeInsets.all(20),
                                child: Column(
                                  ...
                                  children: [
                                    Row(
                                      ...
                                      children: [
                                        ...
                                        Expanded(child: Text(widget.surah.versesAr[index], textDirection: TextDirection.rtl, style: GoogleFonts.amiri(fontSize: 28, color: isHighlighted ? const Color(0xFFD4AF37) : Colors.white, height: 2.0, fontWeight: isHighlighted ? FontWeight.bold : FontWeight.normal))),
                                      ],
                                    ),
                                    const SizedBox(height: 16),
                                    if (translatedVerse.isNotEmpty) Text(translatedVerse, textDirection: TextDirection.ltr, style: GoogleFonts.elMessiri(fontSize: 16, color: isHighlighted ? Colors.white : Colors.white.withValues(alpha: 0.75), height: 1.6, fontStyle: FontStyle.italic)),
                                  ],
                                ),
                              );
                            }),
                ...
                ClipRRect(
                  child: BackdropFilter(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(color: const Color(0xFF0B3D2E).withValues(alpha: 0.8), border: Border(top: BorderSide(color: const Color(0xFFD4AF37).withValues(alpha: 0.3)))),
                      child: Column(
                        children: [
                          Container(
                            padding: const EdgeInsets.symmetric(horizontal: 16),
                            decoration: BoxDecoration(color: Colors.black.withValues(alpha: 0.2), borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0xFFD4AF37).withValues(alpha: 0.5))),
                            child: DropdownButtonHideUnderline(
                              child: DropdownButton<QuranReciter>(
                                ...
                                items: QuranReciterService.reciters.map((reciter) { return DropdownMenuItem(value: reciter, child: Text(widget.lang == 'ar' ? reciter.nameAr : reciter.nameEn, style: GoogleFonts.elMessiri(color: const Color(0xFFD4AF37), fontSize: 16))); }).toList(),
                          ...
                          if (_duration != Duration.zero)
                            Column(
                              children: [
                                SliderTheme(data: SliderThemeData(activeTrackColor: const Color(0xFFD4AF37), inactiveTrackColor: Colors.white.withValues(alpha: 0.2), thumbColor: const Color(0xFFD4AF37), overlayColor: const Color(0xFFD4AF37).withValues(alpha: 0.3), trackHeight: 4), child: Slider(value: _position.inSeconds.toDouble(), max: _duration.inSeconds.toDouble(), onChanged: (value) => QuranReciterService.seek(Duration(seconds: value.toInt())))),
                                Padding(padding: const EdgeInsets.symmetric(horizontal: 12), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(_formatDuration(_position), style: const TextStyle(color: Colors.white70, fontSize: 13)), Text(_formatDuration(_duration), style: const TextStyle(color: Colors.white70, fontSize: 13))])),
                              ],
                            ),
                        ],
                      ),
